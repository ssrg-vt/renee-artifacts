...
class pvs_lemma_generator:
    ...
    def generate_pvs_lemma_file(self, post_state):
        self.f.write(self.pvs_lemma_filename)
        self.f.write(" : THEORY\n")
        self.f.write("\n      BEGIN\n")
        self.f.write("\n      IMPORTING rsl@str_imm_gen, bitvectors@bv_arith_nat\n")

        self.f.write("\n      X_sts : [ below(32) -> bvec[64] ] = init`X with [( " + self.Rn + " ) := " + self.Rn_val[0] + ",\n\t\t\t\t\t\t\t ( " + self.Rt + " ):= " + self.Rt_val[0] + "] \n")

        self.f.write("\n      p     : s = init with [`X:= X_sts]\n")
        self.f.write("\n      conc_test_1 : Theory = str_imm_gen[p]{{ Diag:= bv[32](0b" + self.input_bitstring + ") , addr:= " + self.R_PC[0] + "}}\n")
        
        self.f.write("\n      u1 : VAR nat\n")
        self.f.write("\n      test1 : lemma conc_test_1.u = u1 => \n")
        self.f.write("                          let Mem_post = p`Mem with [ ( " + str(post_state) + "):= " + "p`X(" + self.Rt + ") ] in\n")
        self.f.write("                                         conc_test_1.post`Mem(u1) = Mem_post(" +  str(post_state) + ")\n")
        # self.f.write(
        #     "\n                    let p2     = p with [`Mem:= Mem_post] in \n") #% `NZVC:= " + self.R_NZCV[1] + " , `PC:= " + self.R_PC[1] + "] in\n")
        # self.f.write("\n                                 %conc_test_1.post`X( " + self.Rn + " ) = p2`X( " + self.Rn + " ) and\n")
        # self.f.write("\n                                 conc_test_1.post`Mem( " + str(post_state) + ", 8, AccType_NORMAL) = p`X( " + self.Rn + " )")
...
if __name__ == '__main__':
    ...
    # Run the instruction
    post_state = mtest.test_arm64(bytes(bytecode), op_fileptr, reg_list, bitstring)

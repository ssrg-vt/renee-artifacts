...
def test_arm64(ARM64_CODE, output_file, reg_list, bitstring):
    ...
    reversed_offset = offset[::-1]
    new_offset = "0b" + reversed_offset
    offset_to_hex = subprocess.check_output(["rax2", new_offset])[:-1]
    offset_to_int = int(offset_to_hex, 16)
    final_offset = offset_to_int << 3
    print ("final_offset ", final_offset)
    post_state = ""
    final_address = ""
    ...
    try:
        ...
        if Rn_val_hex >= 0x10000 and Rn_val_hex <= 0x200000 and Rn_val % 8 == 0 and (Rn_val_hex + offset_to_hex) >= 0x10000 and (Rn_val_hex + offset_to_hex) <= 0x200000 and (Rn_val + offset_to_hex) % 8 == 0:
            ...
            final_address = str(Rn_val_hex + final_offset)
            ...
            # print ("offset to integer = ", offset_to_int)
            print ("Rn_val = ", Rn_val)
            final_address = str(Rn_val + final_offset)
        ...
        return post_state, final_address
    except UcError as e:
        print("ERROR: %s" % e)

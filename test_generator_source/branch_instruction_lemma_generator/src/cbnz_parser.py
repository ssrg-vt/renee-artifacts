...
def test_lemma_generator_with_bitvector():
	...
	for pvsFileName in batchPtr:
		...
		if bool(instDict): 
			...
			jump = ""
			fail = ""
			print (" *********************** jump fail dict ********************************")
			print (jumpFailDict)
			if 'jump' in jumpFailDict and 'fail' in jumpFailDict:
				print ("\nboth present")
				jump = str(jumpFailDict['jump'])
				fail = str(jumpFailDict['fail'])
			elif 'jump' in jumpFailDict and 'fail' not in jumpFailDict:
			...
			if 'jump' in jumpFailDict and 'fail' in jumpFailDict:
				...
				secondPart = 'next:= ' + jumpFailDict['jump'] + ' }}'
			elif 'fail' in jumpFailDict and 'jump' not in jumpFailDict:
				secondPart = 'fail:= ' + jumpFailDict['fail'] + ' }}'
			...
			f.write("\n%|- p_TCC1         : PROOF")
			f.write("\n%|- " + instructionName + "_TCC*: PROOF (eval-formula)")
			# f.write("\n%|- test1_TCC1      : PROOF (eval-formula)")
			f.write("\n%|- QED")

...
class pvs_lemma_generator:
    ...
    def generate_pvs_lemma_file(self):
        ...
        self.f.write(
            "\n      p    :  s = init with [`X:= X_sts, `PSTATE`NZCV:= " + self.R_NZCV[0] + "] %," "`PC:= " + self.R_PC[0] + "]\n")
        self.f.write("\n      conc_test_1: Theory = and_log_imm[p]{{ Diag := bv(0b" + self.input_bitstring + ") , addr := " + self.R_PC[0] + "}}\n")

        self.f.write(
            "\n      test1: lemma let X_post =  p`X with [ ( " + self.Rd + " ) " + ":= " + self.R_op_val + "] in\n")
        self.f.write(
            "\n          let p2     =  p with [`X:= X_post, `PSTATE`NZCV := " + self.R_NZCV[1] + "] in % , `PC:= " + self.R_PC[1] + "] in\n")
        self.f.write("\n                        conc_test_1.post`X( " + self.Rd + " ) = p2`X( " + self.Rd + " ) and\n")
        self.f.write("\n                        conc_test_1.post`PSTATE`NZCV = p2`PSTATE`NZCV \n")

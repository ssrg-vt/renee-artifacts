...
class inst_generator:
    ...
    def get_registers(self):
        reg_list = [str(self.Rd), str(self.Rn)]
        return reg_list
...
def test_arm64(ARM64_CODE, output_file, reg_list, nzcv_param):
    ...
    try:
        ...
        print(
            "R_NZCV_pre=bv(0b" + str(bin(int(nzcv_val))[2:].zfill(64))[32:36] + ")")
        print("Rd_pre (without reading from unicorn)=" +
              convert_to_bvec(str(Rd_val), 64))
        ...
        output_file.write(
            "Rd_post=bv(0b" + str(bin(Rd_val)[2:].zfill(64)[::-1]) + ")\n")

        output_file.write(
            "Rn_post=bv(0b" + str(bin(Rn_val)[2:].zfill(64)[::-1]) + ")\n")
        ...
        print("R_NZCV_post=bv(0b" + str(bin(nzcv_post)
                                        [2:].zfill(32)[0:4]) + ")" + "\n")
        output_file.write(
            "R_NZCV_post=bv(0b" + str(bin(nzcv_post)[2:].zfill(32)[0:4]) + ")" + "\n")
        print ("R_op=" + Rd)

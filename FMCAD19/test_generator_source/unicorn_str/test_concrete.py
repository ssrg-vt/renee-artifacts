...
# THIS IS A FILE WITH CODE EDITED FOR TESTING PURPOSES WITH A HARDCODED VALUE 01000010000000001010000011010111. Use testcode to search for all changes that were made
...
import re
import os
import operational_addsub_imm as op


class test_concrete:

    def __init__(self, bitstring, hexstring):
        ...
        self.cmdline_input = sys.argv[2]
        self.args = re.split("\s+", self.cmdline_input)
        self.extraction_class_name = sys.argv[2]
        ...
        # testcode holds a mapping from the register name to the value/ "Rd:5"
        self.register_value_map = {}
        self.input_bitstring = bitstring
        ...
        self.theory_name = "concrete_test_" + \
            self.inst_file_name.replace(".xml", "") + "_" + hexstring.upper()
        self.output_file = "concrete_test_" + \
            self.inst_file_name.replace(
                ".xml", "") + "_" + hexstring.upper() + ".pvs"
        self.proof_strategy = None
        # List for translating the declaration part from ASL to PVS
        self.ASL2PVS_types_map = []
        # List for translating the function names in the declaration part from ASL to PVS
    ...
    def generate_pvs_code(self):
        ...
        self.file_ptr = open(output_dir + self.output_file, "w")
        self.extract_field()
        ...
        self.operational_addsubshifted()
        self.end_theory()
    ...
    def generate_theory_name(self):
        ...
        theory_param = ""
        self.file_ptr.write(
            self.theory_name + "[" + arm_state_param + theory_param + "]" + ":THEORY\n")
    ...
    def create_pvs_var_proof(self, var_name, var_type, *positional_parameters, **keyword_parameters):
        ...
        # Amer used to be " : " this is a syntax error here in pvs.
        pvs_var = var_name + ":= " + var_type + option
        return pvs_var
    ...
    def create_pvs_record_type(self, param_list):
        ...
        for x in param_list:
            if record_type == "[# ":
                ...
                record_type = record_type + ", " + x
        # Amer: used to be n=32, give two spaces around equality to avoid some errors in pvs7
        record_type = record_type + " ,length: {n: nat|n = 32} #]"
        #print record_type
        return record_type
    ...
    def create_pvs_proof(self, param_list):
        ...
        for x in param_list:
            if cnt == 0:
                ...
                record_type = record_type + ", " + x
            cnt += 1
        # Amer: used to be length: = 32, the space here is an error.
        record_type = record_type + ", length:= 32 #)\")"

        return record_type

    # testcode Auto-generates the diagram from the xml file to the pvs
    def generate_inst_diagram_pvs(self):
        ...
        # testcode
        self.file_ptr.write(
            "\n\t" + self.generate_diagram_with_bvec(self.input_bitstring) + "\n")
        ...
        self.file_ptr.write("\n%|- diag_TCC* : PROOF " +
                            self.proof_strategy + " QED\n")
        self.file_ptr.write("\n%|- v_TCC* : PROOF\n")
        self.file_ptr.write("\n%|- p1_TCC*: PROOF (eval-formula) QED\n\n")
        self.file_ptr.write("\n%|- imm_TCC5: PROOF (eval-formula -1) QED\n\n")
        self.file_ptr.write(
            "\n%|- ASL_TCC2: PROOF (then (skosimp*)(subs_addsub_shift_ALS_TCC2)) QED")
    ...
    def generate_field_declaration_pvs(self):
        ...
        for x in self.fields:
            ...
            if 'width' in x:
                ...
                bvec_len = "1"
            field_decl = self.create_pvs_var_unspecified(
                field_name, field_type, length=bvec_len)
    ...
    def generate_proof_fields_pvs(self):
        ...
        for x in self.fields:
            ...
            if 'width' in x:
                ...
                bvec_len = "1"
            field_decl = self.create_pvs_var_proof(
                field_name, field_type, length=bvec_len)
            field_decl_list.append(field_decl)
        return field_decl_list
    ...
    def generate_diagram_init(self):
        ...
        for x in self.fields:
            ...
            if 'width' in x:
                ...
                # Amer: used to be the otherway around, that is unsound. Now it PVS decession precedures can will not generete the v_TCCs at all!
                lowbit = str(lowbit_int) + ", "
            else:
                #print str(x) + "::bt"
                theory_type = "bt"
            str2 = str2 + field_name + ":= " + theory_type + \
                "(Diag, " + lowbit + hibit + " ) ,"
            #print str2
        str3 = ' length:= 32 ]'
        str4 = str1 + str2 + str3
    ...
    # testcode This generates a diagram with the actual bvector values for a given bitstring

    def generate_diagram_with_bvec(self, bitstr):
        str1 = "v: diag = (# "
        str4 = ""
        str2 = ""
        theory_type = ""
        ...
        for x in self.fields:
            ...
            if 'width' in x:
                ...
                #print str(x) + "::bt"
                theory_type = "bt"
            # extract values from bitsring to auto generate the operational part
            if field_name == "Rd" or field_name == "Rn" or field_name == "Rm" or field_name == "imm12":
                # testcode Logic to reverse the string to get the accurate integer value of the
                value_str = bitstr[lowbit_int:int(hibit)+1]
                value_str = value_str[::-1]
                value = int(value_str, 2)
                # insert values into the self.register_value_map to use in operational part of the instructions
                self.register_value_map[field_name] = str(value)

            if lowbit == "":
                str2 = str2 + field_name + ":= " + \
                    "bv( 0b" + bitstr[int(hibit)] + " ) ,"
            else:
                str2 = str2 + field_name + ":= " + \
                    "bv( 0b" + bitstr[lowbit_int:int(hibit)+1] + " ) ,"
        str3 = ' length:= 32 #)'
        str4 = str1 + str2 + str3
        return str4
    ...
    def generate_declarations(self):
        ...
        for line in f:
            ...
            if conversion_fnc in ASL2PVS_fncnames_map:
                ...
                # testcode changed the working of the code for uint
                if conversion_fnc == "UInt":
                    PVS_fnc = "bv2nat"
                    #ASL2PVS_fncnames_map [conversion_fnc] = PVS_fnc
                    # Calculate and add the value for Rn Rm or Rd
                    PVS_fnc = self.register_value_map[extracted_field]
            ...
            if PVS_TYPE == "int":
                if type_name == "datasize":
                    ...
                    # + " (" + extracted_field
                    self.file_ptr.write(
                        "\n\t" + type_name + " : " + PVS_TYPE + " " + operator + " " + PVS_fnc)
            if PVS_TYPE == "bool":
                ...
                field_tokens[2] = "bv ( 0b" + \
                    field_tokens[2].replace("'", "") + " )"
                new_fields = field_tokens[0] + \
                    field_tokens[1] + field_tokens[2]
                self.file_ptr.write("\n\t" + type_name + " : " + PVS_TYPE +
                                    " " + operator + " (" + " " + new_fields + " )")
        self.file_ptr.write(
            "\n\timm : bvec[datasize] = Cond v`shift = bv[2](0b00) -> ZeroExtend(64,12,v`imm12),")
        self.file_ptr.write(
            "\n\t                            v`shift = bv[2](0b10) -> ZeroExtend(64,24,v`imm12 o Zeros(12)), % check order for imm12 o Zeros(12),")
        self.file_ptr.write(
            '\n\t                            Else -> throw("ReservedValue()_AArch64.UndefinedFault()")')
        self.file_ptr.write('\n\t		        EndCond')

    def operational_addsubshifted(self):
        op_addsub_imm = op.operational_addsub_imm(
            self.file_ptr, self.register_value_map["imm12"])
                    ...
                    for child in childnode:
                        if child.tag == "a":
                            if re.search(r'\boptional\b', child.attrib['hover'], re.I):
                                self.theory_params_optional.append(
                                    child.attrib['link'])
                            else:
        ...
        for node in self.inst_root.findall('.//pstext'):
            ...
            for child in node:
                if 'hover' in child.attrib:
                    ...
                    print "\n" + \
                        child.attrib['hover'] + \
                        "\nPath:    /ISA_v83A_A64_xml_00bet5/shared_pseudocode.xml#" + \
                        child.attrib['link']
...
if __name__ == '__main__':	
	inst_imm = test_concrete("10110100111100110100100011010111")

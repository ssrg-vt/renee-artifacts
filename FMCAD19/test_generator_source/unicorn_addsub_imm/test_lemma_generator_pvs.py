...
class pvs_lemma_generator:
    ...
    def generate_pvs_lemma_file(self):
        ...
        self.f.write("\n      X_sts : [ below(32) -> bvec[64] ] = init`X  with [("+self.Rd+"):= " +
                     self.Rd_val[0] + "," + "\n\t\t\t\t\t\t\t   (" + self.Rn + "):= " + self.Rn_val[0] + "] \n")
        self.f.write(
            "\n      p1    :  s = init with [`X:= X_sts, `PSTATE`NZCV:= " + self.R_NZCV[0] + "," "`PC`i:= " + self.R_PC[0] + "]")
        self.f.write("\n     subs_addsub_imm_conc_test: Theory = addsub_imm[p1]{{Diag := bv(0b" + self.input_bitstring + ") , addr:= " + self.R_PC[0] + "}}\n")
        self.f.write(

...
class pvs_lemma_generator:
    ...
    def generate_pvs_lemma_file(self):
        ...
        self.f.write("      : THEORY")
        self.f.write("\n      BEGIN")
        self.f.write("\n      IMPORTING rsl@arm_state, rsl@addsub_imm")
        self.f.write("\n      X_sts : [ below(32) -> bvec[64] ]  = init`X  with [(" + self.Rd + "):= " + self.Rd_val[0] + "," +
                    "\n\t\t\t\t\t\t\t (" + self.Rn + "):= " + self.Rn_val[0] + "] \n")
        self.f.write(
            "\n      p1    :  s = init with [`X:= X_sts, `PSTATE`NZCV:= " + self.R_NZCV[0] + "," "`PC`i:= " + self.R_PC[0] + "]")
        self.f.write("\n      subs_addsub_imm_conc_test : Theory = addsub_imm[p1]{{Diag := bv[32](0b" + self.input_bitstring + ") , addr:= " + self.R_PC[0] + "}}\n")
        self.f.write(
            "\n      test1: lemma let X_post =  p1`X with [ ( " + self.R_op + " ) " + ":= " + self.R_op_val + "] in\n")
        self.f.write(
            "\n          let p2     =  p1 with [`X:= X_post, `PSTATE`NZCV:= " + self.R_NZCV[1] + " , `PC`i:= " + self.R_PC[1] + "] in\n")
        self.f.write("\n          subs_addsub_imm_conc_test.post`X(" + self.R_op + ") = p2`X(" + self.R_op + ") and\n")
        self.f.write("\n          subs_addsub_imm_conc_test.post`PSTATE`NZCV = p2`PSTATE`NZCV \n")

        self.f.write("\n%|- X_sts_TCC*: PROOF")
        self.f.write("\n%|- p1_TCC1   : PROOF")
        self.f.write("\n%|- subs_addsub_imm_conc_test_TCC*: PROOF (eval-formula) QED\n")
        # self.f.write("\n%|-            QED\n")
        # self.f.write("\n\n%|- test1_TCC*: PROOF (skeep)(eval-formula) QED")
        # self.f.write("\n%|- QED\n")
        # Change the instruction sub category label for the intruction of this class
        self.f.write("\n%|- test1 : PROOF")
        self.f.write("\n%|- test1_TCC* : PROOF(skeep)(eval-formula) QED")
        self.f.write("\n%|- QED\n")

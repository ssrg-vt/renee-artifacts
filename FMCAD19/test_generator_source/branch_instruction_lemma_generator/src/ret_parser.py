...
def extract_jump_and_fail_values(addressDict):
	...
	input_file = "../assets/linux/json_files/sym." + jsonFileName + ".json"
	...
	for block in block_list:
		if block["offset"] == offset:
			if 'fail' in block and 'jump' in block:
				jumpFailObj['fail'] = c_ulong(block['fail']).value
				jumpFailObj['jump'] = c_ulong(block['jump']).value
...
def test_lemma_generator():
	...
	for pvsFileName in batchPtr:
		...
		if bool(instDict): 
			...
			outputDir = curDir + "/linux_test_lemmas/"
...
def test_lemma_generator_with_bitvector():
	...
	for pvsFileName in batchPtr:
		...
		if bool(instDict): 
			...
			secondPart = 'addr:= UInt(64, bv[64](0b' + binAddressExtracted + ') }}'

			#delete the string between two strings
			firstPart = re.sub('addr:=.*?}}','',lineFormatted, flags = re.DOTALL)
			...
			if len(jump) > 0 and len(fail) > 0:
				f.write("\n    test1 : lemma " + instructionName + ".post`PC`b = bv[64] (0b" + str(bin(jumpFailDict['fail'])).zfill(64)[2:][::-1] + ") or ")
				f.write("\n                  " + instructionName + ".post`PC`b = bv[64] (0b" + str(bin(jumpFailDict['jump'])).zfill(64)[2:][::-1] + ")\n\n")
			elif len(jump) > 0 and len(fail) <= 0:
				f.write("\n                  " + instructionName + ".post`PC`b = bv[64] (0b" + str(bin(jumpFailDict['jump'])).zfill(64)[2:][::-1] + ")\n\n")
			elif len(jump) <= 0 and len(fail) > 0:
				f.write("\n    test1 : lemma " + instructionName + ".post`PC`b = bv[64] (0b" + str(bin(jumpFailDict['fail'])).zfill(64)[2:][::-1] + ")\n\n")
...
def get_json_name_for_pvs_file(pvsFncName):
	f = open('../linux_terminals_modified_names.json', 'r')
